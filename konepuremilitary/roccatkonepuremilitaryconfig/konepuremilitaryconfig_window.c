...
#include "konepuremilitaryconfig_window.h"
#include "konepuremilitaryconfig_configuration_dialog.h"
...
#define KONEPUREMILITARYCONFIG_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPUREMILITARYCONFIG_WINDOW_TYPE, KonepuremilitaryconfigWindowClass))
#define IS_KONEPUREMILITARYCONFIG_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPUREMILITARYCONFIG_WINDOW_TYPE))
#define KONEPUREMILITARYCONFIG_WINDOW_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPUREMILITARYCONFIG_WINDOW_TYPE, KonepuremilitaryconfigWindowPrivate))

typedef struct _KonepuremilitaryconfigWindowClass KonepuremilitaryconfigWindowClass;
typedef struct _KonepuremilitaryconfigWindowPrivate KonepuremilitaryconfigWindowPrivate;

struct _KonepuremilitaryconfigWindow {
	RoccatConfigWindowPages parent;
	KonepuremilitaryconfigWindowPrivate *priv;
};

struct _KonepuremilitaryconfigWindowClass {
	RoccatConfigWindowPagesClass parent_class;
};

struct _KonepuremilitaryconfigWindowPrivate {
	RoccatKeyFile *config;
	DBusGProxy *dbus_proxy;
};

G_DEFINE_TYPE(KonepuremilitaryconfigWindow, konepuremilitaryconfig_window, ROCCAT_CONFIG_WINDOW_PAGES_TYPE);
...
static gboolean change_profile_handler(gpointer user_data) {
	...
	if (!roccat_handle_error_dialog(GTK_WINDOW(roccat_window), _("Could not activate profile"), &local_error))
		return FALSE;
	konepuremilitary_dbus_emit_profile_changed_outside(KONEPUREMILITARYCONFIG_WINDOW(roccat_window)->priv->dbus_proxy, profile_number);
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	KonepuremilitaryconfigWindow *window = KONEPUREMILITARYCONFIG_WINDOW(roccat_window);
	KonepuremilitaryconfigWindowPrivate *priv = window->priv;
	GError *local_error = NULL;
	guint i;
	KoneplusRmp *rmp;
	RoccatDevice *device;
	KonepuremilitaryconfigProfilePage *profile_page;
	guint actual_profile_index;

	device = roccat_config_window_get_device(roccat_window);

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPUREMILITARYCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(roccat_window), i));
		rmp = konepuremilitaryconfig_profile_page_get_rmp(profile_page);
		konepuremilitary_rmp_update_from_device(rmp, device, i);
		konepuremilitaryconfig_profile_page_set_rmp(profile_page, rmp);
...
static KoneplusRmp *get_rmp(KonepuremilitaryconfigWindow *window, KonepuremilitaryconfigProfilePage *profile_page) {
	KoneplusRmp *rmp;
	gboolean moved;
...
static void set_rmp(KonepuremilitaryconfigWindow *window, guint profile_index, KoneplusRmp *rmp) {
	KonepuremilitaryconfigProfilePage *profile_page = KONEPUREMILITARYCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), profile_index));
	guint profile_number = profile_index + 1;
	konepuremilitaryconfig_profile_page_set_rmp(profile_page, rmp);
	roccat_config_window_pages_set_page_unmoved(ROCCAT_CONFIG_WINDOW_PAGES(window), ROCCAT_PROFILE_PAGE(profile_page));
	konepuremilitary_dbus_emit_profile_data_changed_outside(window->priv->dbus_proxy, profile_number);
}

static gboolean save_single(KonepuremilitaryconfigWindow *window, KoneplusRmp *rmp, guint profile_index, GError **error) {
	RoccatDevice *device = roccat_config_window_get_device(ROCCAT_CONFIG_WINDOW(window));

	if (device)
		konepuremilitary_rmp_save(device, rmp, profile_index, error);
	else
		konepuremilitary_rmp_save_actual(rmp, profile_index, error);
...
static gboolean save_all(KonepuremilitaryconfigWindow *window, gboolean ask) {
	...
	modified = FALSE;
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPUREMILITARYCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
...
static gboolean delete_event_cb(GtkWidget *window, GdkEvent *event, gpointer user_data) {
	return !save_all(KONEPUREMILITARYCONFIG_WINDOW(window), TRUE);
}

static void add_file_filter_to_file_chooser(GtkFileChooser *chooser) {
	GtkFileFilter *filter;
	filter = gtk_file_filter_new();
	gtk_file_filter_set_name(filter, _("KonePureMilitary Windows profiles"));
...
static gchar *konepuremilitary_create_filename_proposition(KoneplusRmp *rmp, guint profile_index) {
	gchar *profile_name;
	gchar *filename;
...
static void save_profile_to_file_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	KonepuremilitaryconfigWindow *window = KONEPUREMILITARYCONFIG_WINDOW(user_data);
	KonepuremilitaryconfigWindowPrivate *priv = window->priv;
	GtkWidget *dialog;
	gchar *filename, *path;
	gchar *temp_filename;
	...
	rmp = konepuremilitaryconfig_profile_page_get_rmp(KONEPUREMILITARYCONFIG_PROFILE_PAGE(profile_page));
	...
	filename = konepuremilitary_create_filename_proposition(rmp, profile_index);
...
static void load_profile_from_file_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		rmp = koneplus_rmp_read_with_path(filename, konepuremilitary_rmp_defaults(), &error);
		if (error) {
			roccat_warning_dialog(GTK_WINDOW(dialog), _("Could not load profile"), error->message);
			g_error_free(error);
		} else {
			koneplus_rmp_set_modified(rmp);
			konepuremilitaryconfig_profile_page_set_rmp(KONEPUREMILITARYCONFIG_PROFILE_PAGE(profile_page), rmp);
			koneplus_rmp_free(rmp);
		}
...
static void save_all_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	save_all(KONEPUREMILITARYCONFIG_WINDOW(roccat_window), FALSE);
}
...
static void menu_edit_preferences_cb(GtkMenuItem *item, gpointer user_data) {
	KonepuremilitaryconfigWindow *window = KONEPUREMILITARYCONFIG_WINDOW(user_data);
	KonepuremilitaryconfigWindowPrivate *priv = window->priv;
	konepuremilitaryconfig_configuration_dialog(GTK_WINDOW(window), priv->config);
	konepuremilitary_dbus_emit_configuration_changed_outside(priv->dbus_proxy);
}
...
static void menu_device_firmware_update_cb(GtkMenuItem *item, gpointer user_data) {
	...
	konepuremilitary = roccat_config_window_get_device(ROCCAT_CONFIG_WINDOW(window));
	version = koneplus_firmware_version_read(konepuremilitary, &local_error);
	if (local_error) {
	...
	assistant = roccat_update_assistant_new(GTK_WINDOW(window), konepuremilitary, KONEPLUS_INTERFACE_MOUSE);
	roccat_update_assistant_add_firmware(ROCCAT_UPDATE_ASSISTANT(assistant), _("Firmware"), version_string, KONEPUREMILITARY_FIRMWARE_SIZE, ROCCAT_FIRMWARE_NUMBER_DEFAULT);
...
static void menu_device_dcu_cb(GtkMenuItem *item, gpointer user_data) {
	KonepuremilitaryconfigWindow *window = KONEPUREMILITARYCONFIG_WINDOW(user_data);
	GtkDialog *dialog;
	...
	dialog = GTK_DIALOG(konepuremilitary_dcu_dialog_new(GTK_WINDOW(window), roccat_config_window_get_device(ROCCAT_CONFIG_WINDOW(window))));
...
static void menu_device_reset_cb(GtkMenuItem *item, gpointer user_data) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPUREMILITARYCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
		rmp = konepuremilitary_rmp_load_save_after_reset(device, i, &error);
		konepuremilitaryconfig_profile_page_set_rmp(profile_page, rmp);
		koneplus_rmp_free(rmp);
		konepuremilitary_dbus_emit_profile_data_changed_outside(priv->dbus_proxy, i + 1);
	}
}

GtkWidget *konepuremilitaryconfig_window_new(void) {
	return GTK_WIDGET(g_object_new(KONEPUREMILITARYCONFIG_WINDOW_TYPE,
			"device-name", KONEPUREMILITARY_DEVICE_NAME_COMBINED,
			NULL));
}

static GObject *constructor(GType gtype, guint n_properties, GObjectConstructParam *properties) {
	GObject *obj;
	KonepuremilitaryconfigWindow *window;
	KonepuremilitaryconfigWindowPrivate *priv;
	...
	obj = G_OBJECT_CLASS(konepuremilitaryconfig_window_parent_class)->constructor(gtype, n_properties, properties);
	window = KONEPUREMILITARYCONFIG_WINDOW(obj);
	priv = window->priv;
	roccat_window = ROCCAT_CONFIG_WINDOW(obj);

	priv->config = konepuremilitary_configuration_load();

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = konepuremilitaryconfig_profile_page_new();
		roccat_config_window_pages_append_page(ROCCAT_CONFIG_WINDOW_PAGES(roccat_window), ROCCAT_PROFILE_PAGE(profile_page));

		rmp = konepuremilitary_rmp_load_actual(i);
		konepuremilitaryconfig_profile_page_set_rmp(KONEPUREMILITARYCONFIG_PROFILE_PAGE(profile_page), rmp);
		koneplus_rmp_free(rmp);
	...
	menu_item = GTK_MENU_ITEM(gtk_image_menu_item_new_with_label(_("DCU")));
	g_signal_connect(G_OBJECT(menu_item), "activate", G_CALLBACK(menu_device_dcu_cb), window);
	roccat_config_window_device_menu_append(roccat_window, menu_item);
	...
	/* keep this order */
	priv->dbus_proxy = konepuremilitary_dbus_proxy_new();

	roccat_config_window_set_device_scanner(roccat_window, ROCCAT_DEVICE_SCANNER_INTERFACE(konepuremilitary_device_scanner_new()));
...
static void konepuremilitaryconfig_window_init(KonepuremilitaryconfigWindow *window) {
	window->priv = KONEPUREMILITARYCONFIG_WINDOW_GET_PRIVATE(window);
}

static void finalize(GObject *object) {
	KonepuremilitaryconfigWindowPrivate *priv = KONEPUREMILITARYCONFIG_WINDOW(object)->priv;

	g_clear_pointer(&priv->dbus_proxy, dbus_roccat_proxy_free);
	konepuremilitaryconfig_configuration_save(GTK_WINDOW(object), priv->config);
	g_clear_pointer(&priv->config, koneplus_configuration_free);

	G_OBJECT_CLASS(konepuremilitaryconfig_window_parent_class)->finalize(object);
}

static void konepuremilitaryconfig_window_class_init(KonepuremilitaryconfigWindowClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);

	gobject_class->constructor = constructor;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KonepuremilitaryconfigWindowClass));
}
