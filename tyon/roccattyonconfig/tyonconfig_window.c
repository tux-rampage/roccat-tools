...
G_DEFINE_TYPE(TyonconfigWindow, tyonconfig_window, ROCCAT_CONFIG_WINDOW_PAGES_TYPE);
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	...
	for (i = 0; i < TYON_PROFILE_NUM; ++i) {
		profile_page = TYON_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(roccat_window), i));
...
static void set_rmp(TyonconfigWindow *window, guint profile_index, TyonRmp *rmp) {
	TyonProfilePage *profile_page = TYON_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), profile_index));
	guint profile_number = profile_index + 1;
	tyon_profile_page_set_rmp(profile_page, rmp);
...
static gboolean save_all(TyonconfigWindow *window, gboolean ask) {
	...
	modified = FALSE;
	for (i = 0; i < TYON_PROFILE_NUM; ++i) {
		profile_page = TYON_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
...
static void menu_device_firmware_update_cb(GtkMenuItem *item, gpointer user_data) {
	...
	assistant = roccat_update_assistant_new(GTK_WINDOW(window), device, TYON_INTERFACE_MOUSE);
	g_object_set(G_OBJECT(assistant),
			"wait-prepare", TYON_FIRMWARE_UPDATE_WAIT_PREPARE,
			"wait-init-data", TYON_FIRMWARE_UPDATE_WAIT_INIT_DATA,
			"wait-data", TYON_FIRMWARE_UPDATE_WAIT_DATA,
			"final-write-check", TYON_FIRMWARE_UPDATE_FINAL_WRITE_CHECK,
			NULL);
	roccat_update_assistant_add_firmware(ROCCAT_UPDATE_ASSISTANT(assistant), _("Firmware"), version_string, TYON_FIRMWARE_SIZE, ROCCAT_FIRMWARE_NUMBER_DEFAULT);
