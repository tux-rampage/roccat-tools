...
#include "koneplusconfig_window.h"
#include "koneplusconfig_configuration_dialog.h"
...
#define KONEPLUSCONFIG_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEPLUSCONFIG_WINDOW_TYPE, KoneplusconfigWindowClass))
#define IS_KONEPLUSCONFIG_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEPLUSCONFIG_WINDOW_TYPE))
#define KONEPLUSCONFIG_WINDOW_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEPLUSCONFIG_WINDOW_TYPE, KoneplusconfigWindowPrivate))

typedef struct _KoneplusconfigWindowClass KoneplusconfigWindowClass;
typedef struct _KoneplusconfigWindowPrivate KoneplusconfigWindowPrivate;

struct _KoneplusconfigWindow {
	RoccatConfigWindowPages parent;
	KoneplusconfigWindowPrivate *priv;
};

struct _KoneplusconfigWindowClass {
	RoccatConfigWindowPagesClass parent_class;
};

struct _KoneplusconfigWindowPrivate {
	RoccatKeyFile *config;
	DBusGProxy *dbus_proxy;
};

G_DEFINE_TYPE(KoneplusconfigWindow, koneplusconfig_window, ROCCAT_CONFIG_WINDOW_PAGES_TYPE);
...
static gboolean change_profile_handler(gpointer user_data) {
	...
	if (!roccat_handle_error_dialog(GTK_WINDOW(roccat_window), _("Could not activate profile"), &local_error))
		return FALSE;
	koneplus_dbus_emit_profile_changed_outside(KONEPLUSCONFIG_WINDOW(roccat_window)->priv->dbus_proxy, profile_number);
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	KoneplusconfigWindow *window = KONEPLUSCONFIG_WINDOW(roccat_window);
	KoneplusconfigWindowPrivate *priv = window->priv;
	GError *local_error = NULL;
	guint i;
	KoneplusRmp *rmp;
	RoccatDevice *device;
	KoneplusconfigProfilePage *profile_page;
	guint actual_profile_index;

	device = roccat_config_window_get_device(roccat_window);

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPLUSCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(roccat_window), i));
		rmp = koneplusconfig_profile_page_get_rmp(profile_page);
		koneplus_rmp_update_from_device(rmp, device, i);
		koneplusconfig_profile_page_set_rmp(profile_page, rmp);
...
static KoneplusRmp *get_rmp(KoneplusconfigWindow *window, KoneplusconfigProfilePage *profile_page) {
	KoneplusRmp *rmp;
	gboolean moved;
...
static void set_rmp(KoneplusconfigWindow *window, guint profile_index, KoneplusRmp *rmp) {
	KoneplusconfigProfilePage *profile_page = KONEPLUSCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), profile_index));
	guint profile_number = profile_index + 1;
	koneplusconfig_profile_page_set_rmp(profile_page, rmp);
	roccat_config_window_pages_set_page_unmoved(ROCCAT_CONFIG_WINDOW_PAGES(window), ROCCAT_PROFILE_PAGE(profile_page));
	koneplus_dbus_emit_profile_data_changed_outside(window->priv->dbus_proxy, profile_number);
}

static gboolean save_single(KoneplusconfigWindow *window, KoneplusRmp *rmp, guint profile_index, GError **error) {
	RoccatDevice *device = roccat_config_window_get_device(ROCCAT_CONFIG_WINDOW(window));

	if (device)
		koneplus_rmp_save(device, rmp, profile_index, error);
	else
...
static gboolean save_all(KoneplusconfigWindow *window, gboolean ask) {
	...
	modified = FALSE;
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPLUSCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
...
static gboolean delete_event_cb(GtkWidget *window, GdkEvent *event, gpointer user_data) {
	return !save_all(KONEPLUSCONFIG_WINDOW(window), TRUE);
}
...
static gchar *koneplus_create_filename_proposition(KoneplusRmp *rmp, guint profile_index) {
	gchar *profile_name;
	gchar *filename;
...
static void save_profile_to_file_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	KoneplusconfigWindow *window = KONEPLUSCONFIG_WINDOW(user_data);
	KoneplusconfigWindowPrivate *priv = window->priv;
	GtkWidget *dialog;
	gchar *filename, *path;
	gchar *temp_filename;
	...
	rmp = koneplusconfig_profile_page_get_rmp(KONEPLUSCONFIG_PROFILE_PAGE(profile_page));
	...
	filename = koneplus_create_filename_proposition(rmp, profile_index);
...
static void load_profile_from_file_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		rmp = koneplus_rmp_read_with_path(filename, koneplus_rmp_defaults(), &error);
		if (error) {
			roccat_warning_dialog(GTK_WINDOW(dialog), _("Could not load profile"), error->message);
			g_error_free(error);
		} else {
			koneplus_rmp_set_modified(rmp);
			koneplusconfig_profile_page_set_rmp(KONEPLUSCONFIG_PROFILE_PAGE(profile_page), rmp);
			koneplus_rmp_free(rmp);
		}
...
static void save_all_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	save_all(KONEPLUSCONFIG_WINDOW(roccat_window), FALSE);
}

static void menu_edit_preferences_cb(GtkMenuItem *item, gpointer user_data) {
	KoneplusconfigWindow *window = KONEPLUSCONFIG_WINDOW(user_data);
	KoneplusconfigWindowPrivate *priv = window->priv;
	koneplusconfig_configuration_dialog(GTK_WINDOW(window), priv->config);
	koneplus_dbus_emit_configuration_changed_outside(priv->dbus_proxy);
}
...
static void menu_device_reset_cb(GtkMenuItem *item, gpointer user_data) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPLUSCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
		rmp = koneplus_rmp_load_save_after_reset(device, i, &error);
		koneplusconfig_profile_page_set_rmp(profile_page, rmp);
		koneplus_rmp_free(rmp);
		koneplus_dbus_emit_profile_data_changed_outside(priv->dbus_proxy, i + 1);
	}
}

GtkWidget *koneplusconfig_window_new(void) {
	return GTK_WIDGET(g_object_new(KONEPLUSCONFIG_WINDOW_TYPE,
			"device-name", KONEPLUS_DEVICE_NAME,
			NULL));
}

static GObject *constructor(GType gtype, guint n_properties, GObjectConstructParam *properties) {
	GObject *obj;
	KoneplusconfigWindow *window;
	KoneplusconfigWindowPrivate *priv;
	...
	obj = G_OBJECT_CLASS(koneplusconfig_window_parent_class)->constructor(gtype, n_properties, properties);
	window = KONEPLUSCONFIG_WINDOW(obj);
	priv = window->priv;
	...
	/* keep this order */
	priv->dbus_proxy = koneplus_dbus_proxy_new();

	roccat_config_window_set_device_scanner(roccat_window, ROCCAT_DEVICE_SCANNER_INTERFACE(koneplus_device_scanner_new()));
...
static void koneplusconfig_window_init(KoneplusconfigWindow *window) {
	window->priv = KONEPLUSCONFIG_WINDOW_GET_PRIVATE(window);
}

static void finalize(GObject *object) {
	KoneplusconfigWindowPrivate *priv = KONEPLUSCONFIG_WINDOW(object)->priv;

	g_clear_pointer(&priv->dbus_proxy, dbus_roccat_proxy_free);
	koneplusconfig_configuration_save(GTK_WINDOW(object), priv->config);
	g_clear_pointer(&priv->config, koneplus_configuration_free);

	G_OBJECT_CLASS(koneplusconfig_window_parent_class)->finalize(object);
}

static void koneplusconfig_window_class_init(KoneplusconfigWindowClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);

	gobject_class->constructor = constructor;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KoneplusconfigWindowClass));
}
