...
G_DEFINE_TYPE(KonepureopticalconfigWindow, konepureopticalconfig_window, ROCCAT_CONFIG_WINDOW_PAGES_TYPE);
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPUREOPTICALCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(roccat_window), i));
...
static void set_rmp(KonepureopticalconfigWindow *window, guint profile_index, KoneplusRmp *rmp) {
	KonepureopticalconfigProfilePage *profile_page = KONEPUREOPTICALCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), profile_index));
	guint profile_number = profile_index + 1;
	konepureopticalconfig_profile_page_set_rmp(profile_page, rmp);
...
static gboolean save_all(KonepureopticalconfigWindow *window, gboolean ask) {
	...
	modified = FALSE;
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEPUREOPTICALCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
